...
hal_result_t hal_driver_of1x_process_packet_out(uint64_t dpid, uint32_t buffer_id, uint32_t in_port,
	...
	if (buffer_id && buffer_id != OF1XP_NO_BUFFER) {
		...
		//Buffer has expired
		if (!pkt) {
			ROFL_DEBUG("["DRIVER_NAME"] Unable to recover the buffer\n");
			return HAL_FAILURE; /* TODO: add specific error */
		}
		...
		if (!pkt) {
			ROFL_DEBUG("["DRIVER_NAME"] No available buffers");
...
hal_fm_result_t hal_driver_of1x_process_flow_mod_add(uint64_t dpid, uint8_t table_id, of1x_flow_entry_t** flow_entry,
	...
	if (table_id >= lsw->pipeline.num_of_tables) {
		ROFL_INFO("["DRIVER_NAME"] calling %s(): invalid table id %u\n", __FUNCTION__, table_id);
		return HAL_FM_FAILURE;
	}
	...
	/*if (!is_valid_entry(*flow_entry)) {
		//TODO: Instead of rejecting the entry, add it to the software table if matches or actions aren't supported
		ROFL_INFO("["DRIVER_NAME"] calling %s(): The entry has matches or actions unsupported\n", __FUNCTION__);
		return HAL_FM_FAILURE;
	}*/

	ROFL_INFO("["DRIVER_NAME"] calling %s(): adding entry into LSI table...\n", __FUNCTION__);
	...
	ROFL_INFO("["DRIVER_NAME"] calling %s(): entry added\n", __FUNCTION__);
...
hal_fm_result_t hal_driver_of1x_process_flow_mod_modify(uint64_t dpid, uint8_t table_id, of1x_flow_entry_t** flow_entry,
	...
	ROFL_INFO("["DRIVER_NAME"] calling %s()", __FUNCTION__);
	...
	ROFL_INFO("["DRIVER_NAME"] calling %s(): modifying entry into LSI table...\n", __FUNCTION__);
	...
	ROFL_INFO("["DRIVER_NAME"] calling %s(): entry modified\n", __FUNCTION__);
...
hal_fm_result_t hal_driver_of1x_process_flow_mod_delete(uint64_t dpid, uint8_t table_id, of1x_flow_entry_t* flow_entry,
	...
	if (table_id >= lsw->pipeline.num_of_tables && table_id != OF1X_FLOW_TABLE_ALL) {
		return HAL_FM_INVALID_TABLE_ID_FAILURE;
		ROFL_INFO("["DRIVER_NAME"] calling %s(): invalid table id %u", __FUNCTION__, table_id);
	}
	...
	ROFL_INFO("["DRIVER_NAME"] calling %s(): Remove entry from LSI table...\n", __FUNCTION__);
	...
	ROFL_INFO("["DRIVER_NAME"] calling %s(): entry removed\n", __FUNCTION__);
